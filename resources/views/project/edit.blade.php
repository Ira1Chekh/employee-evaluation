                    ...
                    <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
                        ...
                        <form action="{{ route('projects.update',$project->id) }}" method="POST">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" value="{{ $project->name }}" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 placeholder:text-right focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Name">
                            </div>

                            <div class="mt-4">
                                <label class="block text-sm font-bold text-gray-700" for="start_date">Start date:</label>
                            ...
                            <div class="mt-4">
                                <label class="block text-sm font-bold text-gray-700" for="importance">Importance:</label>
                                <select name="importance"
                                        id="importance"
                                        class="form-select"
                                >
                                    @foreach ($importance as $key => $value)
                                        <option value="{{ $key }}" {{$project->importance === $key  ? 'selected' : ''}}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mt-4">
                                <label class="block text-sm font-bold text-gray-700" for="users">Employees:</label>
                                <select name="users[]"
                                        id="users"
                                        class="form-select"
                                        multiple
